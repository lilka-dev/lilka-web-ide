...
| `blocks.ts` — 34 блоки Blockly | `gen-blocks.mjs` | `lilka-api.json` |
...
Blockly сам робить тягання, перевірку сумісності й перетворення на код —
генератор Lua у нього штатний. Наша частина — блоки для API Лілки, і вони
**згенеровані** з `lilka-api.json`, а не написані руками: інакше розійшлися б
з API при першій же зміні прошивки. На це є перевірка, яка звіряє кожен блок
із специфікацією: чи існує функція, чи збігається кількість аргументів.

**Блоки й код — два різні файли:** `main.blocks` і згенерований `main.lua`.
Blockly не вміє перетворювати код назад у блоки, тож якби вони жили в одному
файлі, правка коду руками мовчки знищила б усю роботу з блоками.

Дві тонкості в генераторі. Перша: прошивка вимагає **обидва** колбеки —
`AbstractLuaRunnerApp::execute()` перевіряє `if not lilka.update or not
lilka.draw` і мовчки не запускає цикл, якщо бракує хоч одного. Для блоків це
пастка, бо скласти саме «малювати» цілком розумно, тож відсутній колбек
дописується порожнім.

Друга: стан кнопок читається **один раз на кадр** і кладеться у змінну. `just_pressed` у прошивці скидається при читанні, тож якби
кожен блок робив власний `controller.get_state()`, спрацював би лише перший.
