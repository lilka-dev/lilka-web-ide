...
| `blocks.ts` — 33 блоки Blockly | `gen-blocks.mjs` | `lilka-api.json` |
...
Одна тонкість у генераторі: стан кнопок читається **один раз на кадр** і
кладеться у змінну. `just_pressed` у прошивці скидається при читанні, тож якби
