...
export const CTRL = {
    ...
    BUTTONS: 16,
    /**
     * Лічильники натискань і відпускань, по одному int на кнопку.
     *
     * Самого рівня кнопки замало. Воркер знімає стан раз на кадр, тож
     * натискання й відпускання, які вклалися в ті самі 33 мс, він побачив би як
     * «нічого не сталося». На залізі короткий дотик не губиться: перехоплювач
     * контролера виставляє justPressed на самому фронті, а getState() лише
     * забирає прапорець. Лічильники ростуть на кожну подію, тож воркер
     * порівнює їх, а не миттєвий знімок, і жоден фронт не пропадає.
     */
    PRESSES: 32,
    RELEASES: 48,
} as const;

export const CTRL_LENGTH = 64;
